    ...
    CudaStreamPtr runtimeStream, SizeType32 maxSequenceLength) const
{
    ...
    runtime::kernels::invokeFill(*sequenceLimitLength, inputLength + maxNewTokens, *stream);

    TensorPtr inputLengths{ITensor::slice(constPointerCast(dJointInput.lengths), batchSlot, 1)};
    runtime::kernels::invokeFill(*inputLengths, inputLength, *stream);
    ...
    for (SizeType32 ti = 0; ti < decoder.getMaxDecodingEngineTokens(); ++ti)
    {
        ...
        {
            runtime::kernels::invokeFill(*finishedSteps, tk::FinishedState::skipDecoding().toUnderlying(), *stream);
        }
    }
    ...
    if (beamWidth > 1)
    {
        ...
        runtime::kernels::invokeFill(
            *IBuffer::slice(cumLogProbs, 1, beamWidth - 1), DecodingOutput::kNegativeInfinity, *stream);
    ...
    runtime::kernels::invokeFill(*outputIdsTileView, endId, *stream);
    ...
    if (beamWidth > 1)
    {
        runtime::kernels::tileTensorInplace(*outputIdsTileView, beamWidth, *stream);
    }
...
void CreateNewDecoderRequests::newRequestSpeculativeDecoding(SizeType32 batchIdx,
    ...
    CudaStreamPtr runtimeStream, CudaStreamPtr decoderStream, SpeculativeDecodingMode const& speculativeDecodingMode,
...
void CreateNewDecoderRequests::newRequestDraftTokensExternal(SizeType32 batchIdx,
    runtime::decoder_batch::Request const& request, SamplingConfig const& samplingConfig,
    DecodingInput& jointDecodingInput, CudaStreamPtr decoderStream) const
{
    TLLM_LOG_TRACE("%s start", __PRETTY_FUNCTION__);
    ...
    runtime::kernels::invokeFill(*useDraftLogitsView, useDraftLogits, *stream);
    ...
    runtime::kernels::invokeFill(*numDraftTokensView, numDraftTokens, *stream);
    ...
    DecodingInput& jointDecodingInput, CudaStreamPtr decoderStream, SizeType32 maxDecodingEngineTokens) const
{
    ...
    // of first decoder
    runtime::kernels::invokeFill(*curTokensPerStepSlice, 1, *stream);
    ...
    runtime::kernels::invokeFill(*targetTokensPerStepSlice, generatedTokensPerEngineStep, *stream);
    ...
    CudaStreamPtr runtimeStream) const
{
    ...
    runtime::kernels::invokeFill(*positionIdsBaseSlice, request.inputLen, *stream);

    TLLM_LOG_TRACE("%s stop", __PRETTY_FUNCTION__);
}

void CreateNewDecoderRequests::newRequestEagle(SizeType32 batchIdx, runtime::decoder_batch::Request const& request,
    runtime::ModelConfig const& modelConfig, DecodingOutput& jointDecodingOutput, CudaStreamPtr runtimeStream) const
{
    TLLM_LOG_TRACE("%s start", __PRETTY_FUNCTION__);
