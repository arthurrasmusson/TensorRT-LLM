    ...
    , mOperatingBeamWidth{getMaxBeamWidth()}
{
    ...
    mCapacityScheduler
        = std::make_unique<CapacityScheduler>(maxBatchSize * mNumMicroBatches, mKvCacheManager, mCrossKvCacheManager,
            mPeftCacheManager, optionalParams.schedulerConfig.getCapacitySchedulerPolicy(), mNumMicroBatches > 1);

    mMicroBatchScheduler
        = std::make_unique<MicroBatchScheduler>(maxBatchSize, maxNumTokens, ctxChunkConfig, maxContextLength);
...
void TrtGptModelInflightBatching::reshapeKvTensors(KVCacheManager const& kvCacheManager)
{
    ...
    for (auto& buffers : mBuffers)
    {
        ...
        buffers->transformerBuffers->reshapeKvTensors(
            getMaxBatchSize(), mOperatingBeamWidth, maxBlocksPerSeq, *mRuntime, kvCacheManager);
    }
}
    ...
    while (true)
    {
        ...
        auto const findDraftRequest = [this, draftRequestId]() -> std::shared_ptr<LlmRequest>
        {
            ...
            for (auto const& requests : {currRequests.contextRequests, currRequests.generationRequests})
            {
                ...
                {
                    if (llmReq->mState == LlmRequestState::kWAITING_TO_SEND_LOGITS
                        && llmReq->mRequestId == draftRequestId)
                    {
                        return llmReq;
                    }
                }
        ...
        draftRequest->mState = LlmRequestState::kGENERATION_COMPLETE;
    ...
    executor::SpeculativeDecodingFastLogitsInfo const& fastLogitsInfo)
{
    ...
    auto tensor = manager.pinnedPool(ITensor::makeShape({dims[0], dims[1]}), nvinfer1::DataType::kFLOAT);
...
void TrtGptModelInflightBatching::forwardSync()
{
    ...
    {
        bool skipDecoderSync = mWorldConfig.isPipelineParallel() && mWorldConfig.isLastPipelineParallelRank();
        skipDecoderSync = skipDecoderSync || (broadcastPostDecoder() && mWorldConfig.isFirstTensorParallelRank());
        if (!skipDecoderSync)
        {
...
void TrtGptModelInflightBatching::forwardAsync(RequestList const& activeRequests)
{
    ...
    try
    {
        ...
        if (!currRequests.empty())
        {
            ...
            if (mKvCacheManager)
            {
                utils::allocateKvCache(currRequests, mKvCacheManager.get(), mCrossKvCacheManager.get());
            }
...
void TrtGptModelInflightBatching::setRuntimeBatchSize(SizeType32 runtimeBatchSize)
{
    mMicroBatchScheduler->setRuntimeMaxBatchSize(runtimeBatchSize);
}
    ...
    executor::ExtendedRuntimePerfKnobConfig const& extendedRuntimePerfKnobConfig)
{
    ...
    for (SizeType32 i = 0; i < mNumBuffers; ++i)
    {
        mBuffers.emplace_back(std::make_shared<RuntimeBuffers>(getMaxBatchSize(), mOperatingBeamWidth,
            getMaxAttentionWindowVec(), getMaxAttentionWindow(), getSinkTokenLen(), extendedRuntimePerfKnobConfig,
            allReduceCommPtrs, *mRuntime, mModelConfig, mWorldConfig, decodingConfig, getMaxNumTokens()));
    }
...
void TrtGptModelInflightBatching::prepareGraph(SizeType32 bufferId, SizeType32 optProfileId)
{
    ...
    if (!cudaGraphOpt.has_value())
    {
        ...
        mBuffers[bufferId]->prepareBuffersForCudaGraph();
    ...
    RequestVector const& contextRequests, RequestVector const& generationRequests, SizeType32 bufferId)
{
    ...
    {
        bool enableLayerWise = request->isLayerWiseKvCacheEnabled() && request->isLastContextChunk();
        if (enableLayerWise)
        {
    ...
    std::unique_ptr<ContextProgress> progress = nullptr;
    if (mBuffers[bufferId]->transformerBuffers.has_value())
    {
        ...
        if (!layerWiseRequests.empty() && !mModelConfig.useCrossAttention())
        {
            ...
            progress = std::make_unique<ContextProgress>(numLayers);
        }
    ...
    if (progress != nullptr)
    {
        receiverThread = std::thread(&CacheTransceiver::respondAndSendLayerWise, mCacheTransceiver.get(),
            std::move(layerWiseRequests), std::move(progress));
    }
...
void TrtGptModelInflightBatching::setupDecoderStep(RequestVector const& contextRequests)
{
    ...
    auto const retrieveDraftLogits = [&](std::shared_ptr<runtime::ITensor> tensor) -> std::shared_ptr<runtime::ITensor>
    {
        auto& manager = mRuntime->getBufferManager();
        ...
        else
        {
            ...
            auto logits = manager.pinnedPool(ITensor::makeShape({dims[0], dims[1]}), nvinfer1::DataType::kFLOAT);
            commSession.bcast(logits->data(), logits->getSizeInBytes(), mpi::MpiType::kUINT8, 0);
            return logits;
        }
    };
    ...
    ScheduledRequests const& scheduledRequests, DecoderFinishedEventPtr const& decoderFinishEvent)
{
    ...
    if (mWorldConfig.isLastPipelineParallelRank())
    {
        ...
        {
            if (mWorldConfig.isFirstTensorParallelRank())
            {
                auto const peerSend = 1;
                asyncHandles.emplace_back(mDecoderBuffers->asyncSend(mMpiCommTensorPara, returnLogProbs,
                    mOperatingBeamWidth, mModelConfig.getSpeculativeDecodingMode().needsKVCacheRewind(), peerSend));
            }
            else
            {
                auto const peerRecv = mWorldConfig.getTensorParallelRank() - 1;
                mDecoderBuffers->recv(mMpiCommTensorPara, returnLogProbs, mOperatingBeamWidth,
                    mModelConfig.getSpeculativeDecodingMode().needsKVCacheRewind(), peerRecv);

                auto const peerSend = mWorldConfig.getTensorParallelRank() + 1;
                if (peerSend != mWorldConfig.getTensorParallelism())
                {
                    asyncHandles.emplace_back(mDecoderBuffers->asyncSend(mMpiCommTensorPara, returnLogProbs,
                        mOperatingBeamWidth, mModelConfig.getSpeculativeDecodingMode().needsKVCacheRewind(), peerSend));
                }
        ...
        for (auto const& llmReq : requests)
        {
            ...
            if (decoderFinishedPtr[seqSlot] != 0U || mModelConfig.getSpeculativeDecodingMode().isDraftTokensExternal())
            {
                ...
                if (!mWorldConfig.isPipelineParallel() || !mWorldConfig.isLastPipelineParallelRank())
                {
                    ...
                    {
                        llmReq->mState = LlmRequestState::kWAITING_TO_SEND_LOGITS;
                    }
                    else
                    {
                        llmReq->mState = LlmRequestState::kGENERATION_COMPLETE;
...
void TrtGptModelInflightBatching::rewindKVCacheBlocks(SizeType32 numSequences)
{
    ...
    auto const localNbLayers = mModelConfig.getNbAttentionLayers(
...
void TrtGptModelInflightBatching::getCurrentIterationStats(executor::IterationStats& stats) const
{
    ...
    stats.maxBatchSizeStatic = mMicroBatchScheduler->getMaxBatchSizeStatic();
    stats.maxBatchSizeTunerRecommended = mMicroBatchScheduler->getMaxBatchSizeTunerRecommended();
    stats.maxBatchSizeRuntime = mMicroBatchScheduler->getMaxBatchSizeRuntime();
