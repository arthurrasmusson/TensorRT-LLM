...
template <typename TLogits>
class DecoderGenerationLogitsTest : public ::testing::Test,
                                    public ::testing::WithParamInterface<TrivialConstantDecoderTestParameters>
{
protected:
    std::unique_ptr<DecoderTestShared<TLogits>> state;

    DecoderGenerationLogitsTest()
    {
        auto const params = GetParam();
        state = SetupDecoderTest<TLogits>(params);
    }

    void runGenerationLogitsTest(TrivialConstantDecoderTestParameters const& parameters)
    {
...
constexpr runtime::SizeType32 kMinMaxBatchSize = 32;
auto const batchSizes = ::testing::Values(kMinMaxBatchSize);

constexpr runtime::SizeType32 kMinNumTopKLogits = 4;
auto const numTopKLogitses = ::testing::Values(kMinNumTopKLogits);
...
using DecoderTopKGenerationLogitsFloatTest = DecoderGenerationLogitsTest<float>;

TEST_P(DecoderTopKGenerationLogitsFloatTest, TestSizeAndValues)
{
    runGenerationLogitsTest(GetParam());
}
